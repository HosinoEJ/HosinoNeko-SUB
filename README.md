...
* `sub.json`: 存放訂閱者名單的 JSON 資料庫。（它馬上就要沒了）
* `.github/workflows/notify.yml`: GitHub Actions 設定檔，應放在目標倉庫裏面

---

## ⚙️ 環境變數設定 (.env)

在部署前，請確保在根目錄建立 `.env` 檔案並將.env.example
...
### 工作流程說明：
每當 `main` 分支下的 `Blog/` 目錄(這個需要你自己修改notify-api.yml)有檔案變動時，Workflow 會：
...
| :--- | :--- | :--- | :--- |
| `GET` | JSON | `/` | 確認項目存活 |
| `POST` | JSON | `/api/webhook` | 接收 GitHub Actions 的通知，執行群發郵件任務。 |
